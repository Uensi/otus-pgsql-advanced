...
- [install_ha.yml](image/install_ha.yml)
- [etcd.yml.j2](image/etcd.yml.j2)
- [configure_etcd.yml](image/configure_etcd.yml)
- [patroni.yml.j2](image/patroni.yml.j2)
- [configure_patroni.yml](image/configure_patroni.yml)
- [haproxy.cfg.j2](image/haproxy.cfg.j2)
- [configure_haproxy.yml](image/configure_haproxy.yml)
- [failover_test.yml](image/failover_test.yml)
- [verify.yml](image/verify.yml)
- [ansible.log](image/ansible.log)
...
+----------+-----------+---------+-----------+----+-------------+-----+------------+-----+
```
![ansible-patroni](image/ansible-patroni.png)

![haproxy-leader](image/haproxy-leader.png)

![balancer-cluster](image/balancer-cluster.png)
...
```text
| ha-pg-01 | 10.1.0.31 | Replica | streaming |    |
...
- [ansible_ha.log](image/ansible_ha.log)

### 8.4. История переключений
...
```bash
psql 'host=158.160.170.211 port=5432 dbname=postgres user=postgres password=1qaz!QAZ' -c "SELECT inet_server_addr() AS backend, pg_is_in_recovery() AS is_replica;"
```
...
| Jinja: `expected token 'end of print statement', got 'qaz'` | пароль внутри `{{ }}` как литерал | пароль вынесен в переменные `all.yml` |
| Зона `ru-central1-c` недоступна | ограничения каталога Yandex Cloud | ВМ в зонах `a, b, d` кворум сохранён |

---

## 11. Что удобно, а что нет
...
1. **HA-кластер развёрнут** в Yandex Cloud с распределением по зонам `a`, `b` и `d` .
...
**Рекомендация:** для продакшна с высоким SLA данный стек хорошее решение, альтернатива Yandex Managed Service for PostgreSQL с HA-опцией.
