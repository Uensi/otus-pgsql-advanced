...
## 4. Настраиваем terraform для YandexCloud
#### 4.1 В каталоге %appdata% создаем файл terraform.rc с содержимым
```
...
#### 4.2 В директории Terraform создаем файлы (locals.tf, main.tf, outputs.tf, providers.tf, variables.tf, terraform.tfvars) --> (по умолчанию C:\Terraform)
...
[terraform_final](terraform_final.png)

#### 4.3 Настраиваем ansible
...
## 6. Механизмы загрузки и их сравнение
...
### 8.1. Время выполнения запросов (среднее по 3 прогонам после warmup)
...
### 8.2. Дополнительное измерение: полный скан таблицы
...
### 8.3. Сводная таблица всех этапов
...
Полные планы — в файлах результатов (`pg_q*_run*.txt`, `ch_q*_plan.txt`, см. Приложение Б). Краткая интерпретация:
...
## 10. Проблемы, с которыми столкнулись
...
**ClickHouse:**
- (+) загрузка и агрегации на порядок-три быстрее;
...
## Приложение А. Хронология эксперимента (UTC, 2026-08-08)
...
- `pg_q1_run1..3.txt`, `pg_q2_run1..3.txt`, `pg_q3_run1..3.txt` — полные планы `EXPLAIN (ANALYZE, BUFFERS, SETTINGS)` PostgreSQL с временами выполнения;
- `ch_q1_run1..3.time`, `ch_q2_run1..3.time`, `ch_q3_run1..3.time` — времена выполнения запросов ClickHouse;
- `ch_q1_plan.txt`, `ch_q2_plan.txt`, `ch_q3_plan.txt` — планы `EXPLAIN PLAN` ClickHouse;
- `pg_schema.sql`, `pg_post_load.sql`, `ch_schema.sql` — DDL;
- `pg_q*_explain.sql`, `ch_q*.sql` — тексты тестовых запросов.

## Приложение В. Инфраструктура и воспроизводимость

- Инфраструктура (2 ВМ, сеть, security groups с доступом по SSH только с IP исследователя) создана через **Terraform**;
- установка СУБД, генерация данных, загрузка, бенчмарк и сбор результатов выполнены через **Ansible** (playbooks: `install_pg.yml`, `install_ch.yml`, `generate_data.yml`, `load_pg.yml`, `load_ch.yml`, `benchmark_pg.yml`, `benchmark_ch.yml`, `fetch_results.yml`);
- эксперимент полностью воспроизводим: `terraform apply` → `ansible-playbook site.yml`.
